...
```
The difference between console.log and return is that console.log prints a given input placed in the paranthesis after console.log(``) into the terminal/node window, in the context of functions, console.log does not save any information placed in it, and will be forgotten. Return will save the information placed afterwords and will be attatched to said function.
```

**_What's the difference between defining and calling a function?_**

```
Defining a function only "gives it a name" calling a function will run whatever you have called.
```
...
```
A data type that contains a set of numbers, or letters, this datatype only represents a label and will not have a `real value`, in the case of numbers i.e. '5'+'5'= 55 so it will produce incrorrect mathmatical outputs.
```

**_Number_**

```
A value that can be used to calculate using arithmitic, or can be used to represent index values.
```

**_Boolean_**

```
A Boolean is a term for the CS principle of binary or 1s and 0s and is represented as a true/false statement or as numbers as mentioned before.
```

**_Array_**

```
An array is a group of strings that is in a ordered list separated by commas, this order is represented by indexes as a number i.e. 0 1 2 3...
```
